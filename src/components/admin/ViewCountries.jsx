...
import {filterCountriesBySearch, filterCountriesByCouncil} from '../../utils/helpers.js'
...
const ViewCountries = () => {
  ...
  <FormModal open={isEditOpen} setOpen={setIsEditOpen} title={`Modify country ${openedCountry?.name}`} description={`Make changes to country ${openedCountry?.name}.`} onSubmit={onSaveChanges}>
    ...
    <Button variant={"secondary"} className="mr-5" onClick={() => {selectUnselect ? setSelectedCountries(filterCountriesByCouncil(filterCountriesBySearch(countriesData, searchBar),selectedCouncilIds).map(c=>c.country_id)) : setSelectedCountries([]); setSelectUnselect(prev => !prev)}}>{selectUnselect ? 'Select All' : 'Unselect'}</Button>
    {(!selectUnselect || selectedCountries.length > 0) && (<Button variant={"primary"} onClick={() => onDeleteCheckbox()}>Delete</Button>)}
    </div>
    <div className=''>
      {filterCountriesByCouncil(filterCountriesBySearch(countriesData, searchBar),selectedCouncilIds).map((country, index) => (
